...
def should_advance_to_profile(profile: UserProfile) -> bool:
    """
    Check if we have enough info to move past welcome.
    Welcome stage advances after the first user message.
    """
    return True  # Always advance from welcome after first interaction
...
def determine_next_stage(
    ...
    Uses heuristic rules to decide whether to advance. The LLM-based
    stage transition in the workflow can override this for edge cases.
