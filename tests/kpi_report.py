...
from src.agent.extractors.needs_extractor import extract_explicit_needs  # noqa: E402
from src.agent.extractors.profile_extractor import extract_profile  # noqa: E402
from src.agent.extractors.reservation_extractor import extract_reservation  # noqa: E402
...
from src.agent.stages import determine_next_stage  # noqa: E402
...
def generate_report():
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.startswith("backend.app.") or alias.name == "app":
                        violations += 1
            elif isinstance(node, ast.ImportFrom):
                if node.module and (node.module.startswith("backend.app.") or node.module == "app"):
                    violations += 1
    ...
    try:
        for mod in [
            "src.agent.sales_agent",
            "src.agent.schemas",
            "src.agent.stages",
            "src.agent.memory",
            "src.agent.response_generator",
        ]:
