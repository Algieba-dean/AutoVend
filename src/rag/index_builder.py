...
class IndexBuilder:
    ...
    def __init__(
        ...
        self.vector_store = ChromaVectorStore(
            persist_directory=self.persist_directory, collection_name=self.collection_name
        )
    ...
    @log_performance
    def build_index(
        self, force_rebuild: bool = False, validate_data: bool = True, parallel_loading: bool = True
    ) -> Dict[str, Any]:
        """
        构建向量索引
    ...
    def _generate_embeddings(self, vehicles: List[Vehicle]) -> List[List[float]]:
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            try:
                ...
                self.logger.info(
                    f"嵌入生成进度: {progress:.1f}% ({i + len(batch_texts)}/{len(texts)})"
                )
    ...
    def _store_vehicles(self, vehicles: List[Vehicle], embeddings: List[List[float]]) -> None:
        ...
        try:
            self.vector_store.add_vehicles(
                vehicles=vehicles, embeddings=embeddings, batch_size=self.batch_size
            )
    ...
    def _validate_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 测试查询
            test_queries = ["丰田SUV", "家用轿车", "新能源车", "商务MPV"]
            ...
            for query in test_queries:
                try:
                    ...
                    if results["ids"] and results["ids"][0]:
                        test_results.append(
                            {
                                ...
                                "top_similarity": 1.0 - results["distances"][0][0]
                        ...
                        test_results.append(
                            {"query": query, "result_count": 0, "top_similarity": 0.0}
                        )

                except Exception as e:
                    test_results.append(
                        {"query": query, "result_count": 0, "top_similarity": 0.0, "error": str(e)}
                    )
            ...
            return {
                ...
                "avg_similarity": sum(r["top_similarity"] for r in test_results)
                / len(test_results),
            }
    ...
    def update_index(self, new_vehicles: List[Vehicle], incremental: bool = True) -> Dict[str, Any]:
        """
        增量更新索引
