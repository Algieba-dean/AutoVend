...
class VehicleDataLoader:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        # 统计信息
        self.stats = {"total_files": 0, "loaded_files": 0, "failed_files": 0, "loading_time": 0.0}
    ...
    def filter_vehicles_by_price_range(
        self, min_price: int, max_price: int, vehicles: List[Vehicle], tolerance: float = 0.2
    ) -> List[Vehicle]:
        """
        按价格区间过滤车辆
        ...
        for vehicle in vehicles:
            ...
            # 检查价格区间是否有重叠
            if vehicle_max >= min_price * (1 - tolerance) and vehicle_min <= max_price * (
                1 + tolerance
            ):
