...
class AutoVend:
    ...
    def __init__(self, api_key=None, model=None, extractor_type="traditional"):
        ...
        if self.extractor_type == "llm":
            self.profile_extractor = ProfileExtractor(api_key=self.api_key, model=self.model)
            self.expertise_evaluator = ExpertiseEvaluator(api_key=self.api_key, model=self.model)
            self.explicit_needs_extractor = ExplicitNeedsExtractor(api_key=self.api_key, model=self.model)
            self.implicit_needs_inferrer = ImplicitNeedsInferrer(api_key=self.api_key, model=self.model)
            self.test_drive_extractor = TestDriveExtractor(api_key=self.api_key, model=self.model)
            ...
            raise ValueError(f"Invalid extractor_type: {self.extractor_type}. Must be 'llm' or 'traditional'.")

        self.conversation_module = ConversationModule(api_key=self.api_key, model=self.model)
    ...
    def generate_response_llm(
        ...
        if expertise_info:
             if isinstance(expertise_info, str):
                 ...
                 self.status_component.update_profile({"expertise": expertise_info["expertise"]})
        
        if explicit_needs_info: self.status_component.update_explicit_needs(explicit_needs_info)
        if implicit_needs_info: self.status_component.update_implicit_needs(implicit_needs_info)
        if test_drive_info: self.status_component.update_test_drive_info(test_drive_info)
        ...
        if user_message.lower() == "Hi AutoVend".lower():
            ...
            normal_welcome_message = self.conversation_module.generate_welcome_message(self.status_component.user_profile)
            begin_profile_message = self.conversation_module.generate_profile_response(
                user_message, self.status_component.user_profile, {}, {}, {}, [], [], "profile_analysis"
            )
            self.status_component.update_stage("profile_analysis")
            autovend_response = f"{normal_welcome_message}\\n{begin_profile_message}"
            return (
                autovend_response, self.status_component.stage, self.status_component.user_profile,
                self.status_component.needs, self.status_component.matched_car_models, self.status_component.test_drive_info
            )
        ...
        if self.status_component.stage["current_stage"] == "profile_analysis":
            if self.status_component.is_all_basic_info_done():
                ...
                profile_message = self.conversation_module.generate_profile_response(
                    user_message, self.status_component.user_profile, self.status_component.needs["explicit"],
                    self.status_component.needs["implicit"], self.status_component.test_drive_info,
                    self.status_component.matched_car_models, self.status_component.matched_car_model_infos,
                    self.status_component.stage["current_stage"]
                )
                return (
                    profile_message, self.status_component.stage, self.status_component.user_profile,
                    self.status_component.needs, self.status_component.matched_car_models, self.status_component.test_drive_info
                )

        
        formatted_history_raw = self.conversation_module.get_history_for_llm_arbitrator().copy()
        ...
        if might_new_stage == "needs_analysis":
            ...
            autovend_response = self.conversation_module.generate_response_for_big_needs_llm(
                user_message, 
                self.status_component.user_profile,
                self.status_component.needs["explicit"],
                self.status_component.needs["implicit"],
    ...
    def generate_response_traditional(
        ...
        future_basic_profile = self.executor.submit(self.basic_profile_extractor.extract_basic_profile, user_message)
        future_additional_profile = self.executor.submit(self.additional_profile_extractor.extract_additional_profile, user_message)
        future_expertise = self.executor.submit(self.expertise_analyst.analyze_expertise, user_message)
        future_explicit_needs = self.executor.submit(self.explicit_in_extractor.extract_explicit_needs, user_message)
        future_implicit_needs = self.executor.submit(self.implicit_deductor.extract_implicit_values, user_message)
        future_test_drive = self.executor.submit(self.reservation_info_extractor.extract_all_info, user_message)
        ...
        if basic_profile_info: self.status_component.update_profile(basic_profile_info)
        if additional_profile_info: self.status_component.update_profile(additional_profile_info)
        if expertise_info: self.status_component.update_profile({"expertise": str(expertise_info)})
        if explicit_needs_info: self.status_component.update_explicit_needs(explicit_needs_info)
        if implicit_needs_info: self.status_component.update_implicit_needs(implicit_needs_info)
        if test_drive_info: self.status_component.update_test_drive_info(test_drive_info)
        ...
        might_new_stage = self.stage_determining_module.determine_stage(
            user_message, self.status_component.user_profile,
            self.status_component.needs["explicit"], self.status_component.needs["implicit"],
            self.status_component.test_drive_info,
        )

        if might_new_stage:
            if self.status_component.stage["current_stage"] not in ["needs_analysis","car_selection_confirmation","implicit_confirmation","model_introduction"] and \
               might_new_stage == "needs_analysis":
                self.status_component.update_stage(might_new_stage)
            
            if self.status_component.stage["current_stage"] not in ["reservation4s","reservation_confirmation"] and \
               might_new_stage == "reservation4s":
                self.status_component.update_stage(might_new_stage)
    ...
    def _handle_common_stage_logic(self, user_message: str):
        ...
        if self.status_component.stage["current_stage"] == "reservation4s":
            if self.status_component.is_all_basic_reservation_info_done():
                ...
                return (
                    reservation_message, self.status_component.stage, self.status_component.user_profile,
                    self.status_component.needs, self.status_component.matched_car_models, self.status_component.test_drive_info
                )
        ...
        if self.status_component.stage["current_stage"] == "reservation_confirmation":
            ...
            return (
                confirmation_message, self.status_component.stage, self.status_component.user_profile,
                self.status_component.needs, self.status_component.matched_car_models, self.status_component.test_drive_info
            )
...
if __name__ == "__main__":
    ...
    print(f"Using extractor type: {assistant.extractor_type}") # Show which type is used
    ...
    current_profile = {
        "phone_number": "", "age": "", "user_title": "", "name": "", "target_driver": "", "expertise": "",
        "additional_information": {"family_size": "", "price_sensitivity": "", "residence": "", "parking_conditions": ""},
        "connection_information": {"connection_phone_number": "", "connection_id_relationship": "", "connection_user_name": ""}
    }
    current_needs = {"explicit": {}, "implicit": {}}
    current_reservation_info = {
        "test_driver": "", "reservation_date": "", "reservation_time": "", "reservation_location": "",
        "selected_car_model": "", "reservation_phone_number": "", "salesman": ""
    }
    current_matched_car_models = []
    
    # Simulate first call with empty message to get initial greeting
    initial_response, current_stage_info, current_profile, current_needs, current_matched_car_models, current_reservation_info = assistant.generate_response(
        "", current_stage_info, current_profile, current_needs, current_matched_car_models, current_reservation_info
    )
    ...
    while True:
        ...
        response, stage_out, profile_out, needs_out, matched_models_out, reservation_out = (
        ...
        if profile_out:
            print(f"User Profile: {json.dumps(profile_out, ensure_ascii=False, indent=2)}")

        if needs_out:
            print(f"Needs: {json.dumps(needs_out, ensure_ascii=False, indent=2)}")
        
        if reservation_out:
            print(f"Reservation Info: {json.dumps(reservation_out, ensure_ascii=False, indent=2)}")
