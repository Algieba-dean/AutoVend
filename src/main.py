...
def search_command(args) -> None:
    ...
    try:
        ...
        logger.info(f"✅ 找到 {len(response.results)} 个匹配结果，耗时 {response.search_time:.2f}秒")
        ...
        for i, result in enumerate(response.results[:args.show], 1):
            ...
            logger.info(f"   车型: {result.vehicle.precise_labels.vehicle_category_bottom or '未知'}")
...
示例用法:
  ...
  python src/main.py test                     # 运行测试
    ...
    # 测试命令
    test_parser = subparsers.add_parser('test', help='运行检索测试')
