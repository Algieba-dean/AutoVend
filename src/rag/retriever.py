...
class VehicleRetriever:
    ...
    def _calculate_match_scores(
        ...
        for i in range(len(vector_results['ids'][0])):
            ...
            explanation = self._generate_explanation(
                query, metadata, semantic_score, price_score, category_score, feature_score
            )
    ...
    def _calculate_category_match(self, query: Query, metadata: Dict[str, Any]) -> float:
        ...
        if query.vehicle_type:
            ...
            if query.vehicle_type.lower() in category.lower() or category.lower() in query.vehicle_type.lower():
                score *= 1.0
            else:
                score *= 0.5
    ...
    def _get_matched_features(self, query: Query, metadata: Dict[str, Any]) -> List[str]:
        ...
        if query.vehicle_type and query.vehicle_type.lower() in metadata.get('category', '').lower():
    ...
    def _get_missing_features(self, query: Query, metadata: Dict[str, Any]) -> List[str]:
        ...
        if query.vehicle_type and query.vehicle_type.lower() not in metadata.get('category', '').lower():
    ...
    def _update_stats(self, response: SearchResponse) -> None:
        ...
        if response.results:
            ...
            # 更新平均相似度
            avg_score = sum(r.score.semantic_score for r in response.results) / len(response.results)
        ...
        self.stats['avg_response_time'] = (
            (self.stats['avg_response_time'] * (self.stats['total_queries'] - 1) + response.search_time) 
            / self.stats['total_queries']
        )
