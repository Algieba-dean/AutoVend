...
from src.agent.voice.asr import TranscriptionResult, TranscriptionSegment, WhisperASR
from src.agent.voice.pipeline import VoicePipeline, VoiceSessionMetrics, VoiceTurnResult
from src.agent.voice.tts import DEFAULT_VOICE, VOICE_EN_FEMALE, EdgeTTSService, TTSResult
...
class TestWhisperASR:
    ...
    @patch("src.agent.voice.asr.WhisperModel")
...
class TestEdgeTTSService:
    ...
    @pytest.mark.asyncio
    @patch("src.agent.voice.tts.edge_tts.Communicate")
...
class TestVoicePipeline:
    ...
    @patch("src.agent.voice.pipeline.asyncio.run")
...
class TestVoiceRoutes:
    ...
    def mock_voice_modules(self):
        ...
        with (
            patch("backend.app.routes.voice._asr") as mock_asr,
            patch("backend.app.routes.voice._tts") as mock_tts,
            patch("backend.app.routes.voice._pipeline") as mock_pipeline,
        ):
            yield mock_asr, mock_tts, mock_pipeline

    def test_voice_session_create(self):
        """Test voice session creation endpoint logic."""
        from backend.app.routes.voice import _voice_sessions
